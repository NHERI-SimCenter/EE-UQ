...
int main(int argc, char** argv) {
  ...
  if (argc == 6) {
    ...
    //
    // if -getRV we want to set EVENT file with random variables and events that
    // just contain name
    //
    ...
    json_array_foreach(eventsArray, index, value) {
      ...
      if (strcmp(eventType, "ExistingSimCenterEvents") != 0) {
        ...
        json_array_append(newEventArray,
                          value);  // copy event for next event app to parse

      } else {

        json_t* eventObj = json_object();
        json_object_set(eventObj, "type", json_string("Seismic"));
        json_object_set(eventObj, "subtype",
                        json_string("MultipleSimCenterEvent"));
        ...
        if (numExisting > 1) {
          ...
          json_object_set(randomVar, "distribution",
                          json_string("discrete_design_set_string"));
          ...
          json_array_foreach(existingEventsArray, index, existingEvent) {
            json_array_append(theMultipleEvents,
                              json_object_get(existingEvent, "name"));
          }
    ...
    // need to open up INPUT file to see the name of this file (it should be in
    // dir, then copy file to replace EVENT
    //
    ...
    json_array_foreach(eventsEventsArray, count, value) {
      ...
      if (strcmp(eventType, "Seismic") == 0) {
        ...
        if ((subType != NULL) && (strcmp("MultipleSimCenterEvent",
                                         json_string_value(subType)) == 0)) {
          ...
          if (index != NULL) {
            if (json_is_integer(index) == false) {
              ...
              for (int i = 0; i < json_array_size(events); i++) {
                ...
                const char* name =
                    json_string_value(json_object_get(theEvent, "name"));
                json_t* factorValue = json_object_get(theEvent, "factor");
                double factor =
                    json_number_value(json_object_get(theEvent, "factor"));
                if (strcmp(eventName, name) == 0) {
                  const char* fileName =
                      json_string_value(json_object_get(theEvent, "fileName"));
              ...
              //
              // we need to go get factor from input file  and set it in the
              // event
              //
